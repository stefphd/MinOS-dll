/*  File main.cpp 
    Main for Bryson-Denham example
...
int main(int argc, char* argv[]) {
    ...
    OCPInterface* ocp;
    try {
        ocp = new OCPInterface("brysonDenham", N, ti, tf);
    ...
    /* Bounds */
    double L = 1.0/9.0; // model parameter
    double lbx[] = { 0, -10 }; // dim nx
    double ubx[] = { L, +10 }; // dim nx
    double lbu[] = { -10 }; // dim nu
    double ubu[] = { +5 }; // dim nu
    double *lbp = NULL; // dim np
    double *ubp = NULL; // dim np
    double *lbc = NULL; // dim nc
    double *ubc = NULL; // dim nc
    double lbb[] = { 0, 1, 0, -1, 0 }; // dim nb
    double ubb[] = { 0, 1, 0, -1, 0 }; // dim nb
    ...
    for (int k = 0; k < N; ++k) {
        x0[2*k] = 0;
        x0[2*k+1] = +1.0 - 2.0*((double) k / (double) (N-1));
        u0[k] = 0;
    }

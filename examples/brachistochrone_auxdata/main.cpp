/*  File main.cpp 
    Main for brachistochrone example with auxdata
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 4) {
        std::cerr << "Too many input arguments. Number of input arguments must be 2." << std::endl;
        return 1;
    }
    ...
    /* Override g if given in input argument */
    double g = 10.0; // default value
    if (argc > 3) { 
        g = atof(argv[3]);
        if (g<=0) {
            std::cerr << "Invalid input arguments. Argument must be a positive number." << std::endl;
    ...
    OCPInterface* ocp;
    try {
        ocp = new OCPInterface("brachistochrone_auxdata", N, ti, tf);
    ...
    double lbb[] = { 0, 0, 0, 2, 2 }; // dim nb
    double ubb[] = { 0, 0, 0, 2, 2 }; // dim nb
    ...
    outfile.open("brachistochrone_auxdata.txt");
    outfile << ocp;
    outfile.close();

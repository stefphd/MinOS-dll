# Generate brachistochrone problem with auxdata
...
g = auxdata[0]
xdot = vertcat(p[0]*x[2]*sin(u[0]), p[0]*x[2]*cos(u[0]), p[0]*g*cos(u[0])) # state derivative
c = [] # path constraint
...
# Boundary conditions
b = vertcat(xi[0], xi[1], xi[2], xf[0], xf[1])

# Integrator (trapezoidal)
h = SX.sym('h')
f = Function('f', [t, x, u, p, auxdata], [xdot, l,  q])
...
# OCP boundary cost
ocp_bcscost = Function('ocp_bcscost', [xi, ui, xf, uf, p, auxdata], [m])
...
ocp_bcs = Function('ocp_bcs', [xi, ui, xf, uf, p, auxdata], [b])

# OCP integral constraints
ocp_int = Function('ocp_int', [t, x1, u, x2, p, h, auxdata], [dq])

# Create gradients
ocp_runcost_grad = Function('ocp_runcost_grad', [t, x1, u, x2, p, h, auxdata], 
                            [gradient(ocp_runcost(t, x1, u, x2, p, h, auxdata), vertcat(x1, u, x2, p))])
ocp_bcscost_grad = Function('ocp_bcscost_grad', [xi, ui, xf, uf, p, auxdata], 
                            [gradient(ocp_bcscost(xi, ui, xf, uf, p, auxdata), vertcat(xi, ui, xf, uf, p))])

# Create jacobians
ocp_dyn_jac = Function('ocp_dyn_jac', [t, x1, u, x2, p, h, auxdata], 
                        [jacobian(ocp_dyn(t, x1, u, x2, p, h, auxdata), vertcat(x1, u, x2, p))]) # include also auxdata
ocp_path_jac = Function('ocp_path_jac', [t, x, u, p, auxdata], 
                        [jacobian(ocp_path(t, x, u, p, auxdata), vertcat(x, u, p))])
ocp_bcs_jac = Function('ocp_bcs_jac', [xi, ui, xf, uf, p, auxdata], 
                        [jacobian(ocp_bcs(xi, ui, xf, uf, p, auxdata), vertcat(xi, ui, xf, uf, p))])
ocp_int_jac = Function('ocp_int_jac', [t, x1, u, x2, p, h, auxdata], 
                        [jacobian(ocp_int(t, x1, u, x2, p, h, auxdata), vertcat(x1, u, x2, p))])
...
if not skip_hessian:
    ...
    # build lagragians
    lagb = sigma*ocp_bcscost(xi, ui, xf, uf, p, auxdata) # boundary lagragian
    lagi = sigma*ocp_runcost(t, x1, u, x2, p, h, auxdata) # internal lagragian
    if nb > 0: # add bcs lagragian
        lagb += dot(lamb, ocp_bcs(xi, ui, xf, uf, p, auxdata))
    if nx > 0: # add dynamic lagragian
        lagi += dot(lamf, ocp_dyn(t, x1, u, x2, p, h, auxdata)) # include also auxdata
    if nc > 0: # add path lagragian
        lagb += dot(lamc, ocp_path(t, xf, uf, p, auxdata))
        lagi += dot(lamc, ocp_path(t, x1, u, p, auxdata))
    if nq > 0: # add integral laggragian
        lagi += dot(lamq, ocp_int(t, x1, u, x2, p, h, auxdata))
    # generate hessians of lagragians
    hessb, _ = hessian(lagb, vertcat(xi, ui, xf, uf, p))
    hessi, _ = hessian(lagi, vertcat(x1, u, x2, p))
    # create CASADI functions
    ocp_hessb = Function('ocp_hessb', [t, xi, ui, xf, uf, p, sigma, lamc, lamb, auxdata], [tril(hessb)]) # include also auxdata
    ocp_hessi = Function('ocp_hessi', [t, x1, u, x2, p, h, sigma, lamc, lamf, lamq, auxdata], [tril(hessi)]) # include also auxdata
    # append to ocp_funcs

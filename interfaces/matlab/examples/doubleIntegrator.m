...
% Solving options
name = 'doubleIntegrator'; % name of the problem
...
% Boundary conditions
xi = [2; +1]; % initial x1, x2
xf = [0; 0]; % final x1, x2

% Bounds
lbx = [-20; -20]; % lower bound for state
ubx = [+20; +20]; % upper bound for state
lbu = -1; % lower bound for control
ubu = 1; % upper bound for control
lbp = 0; % lower bound for parameter
ubp = 200; % upper bound for parameter
...
% Create CASADI variables ( using time scaled by Tf=p(1) )
...
b = [x0;
     xn;
     un % just to avoid undetermined control at the end
     ];
...
% Clean
% delete(['doubleIntergratorMex.' mexext])
